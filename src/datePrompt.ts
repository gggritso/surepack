 ...
 */
export const datePrompt: (config: DatePromptConfig) => Promise<Date> =
	createPrompt<Date, DatePromptConfig>((config, done) => {
		...
		useKeypress((key, rl) => {
			if (isEnterKey(key)) {
				const inputValue =
					value || (config.default ? format(config.default, "MMM d") : "");
				...
				if (parsed) {
					...
					setError(
						`Invalid date: "${inputValue}". Try formats like "15 Jan" or "Jan 15".`,
					);
				}
			} else {
				setValue(rl.line);
		...
		const defaultHint = config.default
			? ` (${format(config.default, "MMM d")})`
			: "";
		const displayValue =
			status === "done" ? format(parseDate(value)!, "MMM do, yyyy") : value;
